...
static volatile uint8_t led_mode = KR260_RPU_LED_HEARTBEAT;
...
static void send_response(struct rpmsg_endpoint *ept,
			  const struct kr260demo_rpu_msg_header *request,
			  uint32_t dst,
			  uint8_t type, uint32_t status,
			  const void *payload, uint16_t payload_len)
{
	uint8_t tx[KR260_RPU_MAX_FRAME_SIZE] = {0};
	struct kr260demo_rpu_msg_header header;
...
static void send_status(struct rpmsg_endpoint *ept,
			...
			uint32_t dst)
{
	struct kr260demo_rpu_status_payload payload;

	payload.core_id = KR260DEMO_RPU_CORE_ID;
	...
	send_response(ept, request, dst, KR260_RPU_MSG_STATUS, KR260_RPU_STATUS_OK,
...
{
	struct kr260demo_rpu_msg_header request;
	...
	if (request.magic != KR260_RPU_MAGIC) {
		error_count++;
		send_response(ept, &request, src, KR260_RPU_MSG_ERROR,
			      KR260_RPU_STATUS_BAD_MAGIC, NULL, 0);
		return RPMSG_SUCCESS;
	}
	if (request.version != KR260_RPU_VERSION) {
		error_count++;
		send_response(ept, &request, src, KR260_RPU_MSG_ERROR,
			      KR260_RPU_STATUS_BAD_VERSION, NULL, 0);
		return RPMSG_SUCCESS;
	}
	...
	default:
		error_count++;
		send_response(ept, &request, src, KR260_RPU_MSG_ERROR,
			      KR260_RPU_STATUS_BAD_TYPE, NULL, 0);
		break;
	}
...
static int app(struct rpmsg_device *rdev, void *priv)
{
	...
	ML_INFO("Creating RPMsg endpoint %s\r\n", KR260DEMO_RPU_SERVICE_NAME);
	ret = rpmsg_create_ept(&ctrl_ept, rdev, KR260DEMO_RPU_SERVICE_NAME,
...
static void openamp_processing(void *unused_arg)
{
	...
	LPRINTF("Starting %s on R5 core %u\r\n",
		KR260DEMO_RPU_SERVICE_NAME, KR260DEMO_RPU_CORE_ID);
...
static void led_processing(void *unused_arg)
{
	...
	while (1) {
		if (led_mode == KR260_RPU_LED_HEARTBEAT) {
			led_on = !led_on;
			apply_led_state(led_on);
			heartbeat_count++;
			vTaskDelay(pdMS_TO_TICKS(KR260DEMO_RPU_HEARTBEAT_PERIOD_MS));
		} else {
...
int main(void)
{
	...
	status = XGpio_Initialize(&led_gpio, KR260DEMO_RPU_UF2_LED_BASEADDR);
